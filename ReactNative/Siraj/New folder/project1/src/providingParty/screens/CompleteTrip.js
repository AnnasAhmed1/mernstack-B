...
import {TrackNow, BackNav,NotificationNav} from '../components';
...
console.log(item)
        ...
        if(location) {
            ...
            //   if(fareCalculation) {
                ...
                let convenienceFee = (parseFloat(item.estimate)*10/100);
    

    let grandtotal = parseFloat(item.estimate);
                this.finalCostStore(item,grandtotal,pos,0,convenienceFee)
    ...
    finalCostStore(item,finalFare,pos,distance,convenience_fees){
        ...
        let customer_paid = (finalFare + convenience_fees);
        var data = {
            status:"END",
    ...
    return (
        <View style={styles.containerView}>
            <Header 
                ...
                // centerComponent={<Text style={styles.headerTitleStyle}>{"In The Middle Of Work"}</Text>}
