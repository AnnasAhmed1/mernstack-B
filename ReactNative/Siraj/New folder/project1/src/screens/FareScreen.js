        ...
        return (
            <Modal
                ...
                <Header
                    ...
                    rightComponent={{ icon: 'ios-close', type: 'ionicon', color: colors.WHITE, size: 35, component: TouchableWithoutFeedback, onPress: () => { this.setState({ promodalVisible: false }) } }}
                                                    ...
                                                    if (allUsers[key].usertype == 'driver' && allUsers[key].approved == true && allUsers[key].queue == false && allUsers[key].driverActiveStatus == true) {
                                                        if (allUsers[key].location) {
                                                           ...
                                                           // var location2 = [allUsers[key].location.lat, allUsers[key].location.lng];//Driver lat and lang
                                                            ...
                                                            var distance =  getDistance(this.state.region.wherelatitude,this.state.region.wherelongitude, allUsers[key].location.lat,allUsers[key].location.lng );
                                                    ...
                                                    if (allUsers[key].location) {
                                                        ...
                                                        if (originalDistance < 10) {// Request will be send if distance less than 10 km 
                                                            if (allUsers[key].carType == this.state.carType) {
                                                                allUsers[key].driverUid = key;
                                                                arr.push(allUsers[key].driverUid);
                                                               //nomi //console.log("Driver user id",allUsers[key].driverUid);
    ...
    render() {
        return (
            <View style={styles.container}>
                ...
                <Header
                    backgroundColor={colors.GREY.default}
                    leftComponent={{ icon: 'md-menu', type: 'ionicon', color: colors.WHITE, size: 35, component: TouchableWithoutFeedback, onPress: () => { this.props.navigation.toggleDrawer(); } }}
