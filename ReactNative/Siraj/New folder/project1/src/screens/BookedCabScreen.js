...
export default class BookedCabScreen extends React.Component {
  ...
  componentDidMount() {
    ...
    let bookingResponse = firebase.database().ref(`users/` + curuser.uid + '/my-booking/'+this.getParamData.bookingId);
    ...
    bookingResponse.on('value', currUserBookings => {
     if(currUserBookings.val()){
       ...
       this.setState({
            ...
            estimateTime:0,
            currentBookingId:this.getParamData.bookingId,
            currentUser:curuser,
            ...
            estimate: currUserBookings.val().estimate,
            trip_cost: currUserBookings.val().trip_cost,
       ...
       if(currUserBookings.val().status == "ACCEPTED" || currUserBookings.val().status == "Waiting for customer approval..." ){
        this.setState({
            bookingStatus:currUserBookings.val().status,
            driverSerach:false
        })
       }else if(currUserBookings.val().status == "START"){
        this.props.navigation.navigate('trackRide',{data:currUserBookings.val(),bId:this.getParamData.bookingId});
     }
     }
    })
        ...
        this.setState({ modalVisible: false },()=>{
            setTimeout(() => {    
                ...
                firebase.database().ref(`/users/`+this.state.currentUser.uid+'/my-booking/'+this.state.currentBookingId + '/').update({
                    ...
                    //alert(this.state.currentBookingId)
                             ...
                             firebase.database().ref(`/users/`+this.state.driverUID+'/my_bookings/'+this.state.currentBookingId + '/').on('value',curbookingData=>{
                                 if(curbookingData.val()){
                                    //  console.log(curbookingData.val())
                                     if(curbookingData.val().status == 'ACCEPTED' || currUserBookings.val().status == "Waiting for customer approval..."){
...
//ride cancel confirm modal design
  ...
  chat(){
      //item.customer + ',' + firebase.auth().currentUser.uid
    //  alert(this.getParamData.driver)
       const obj = {
         "bookingId": this.getParamData.bookingId,
         "mainKey":firebase.auth().currentUser.uid + ',' + this.getParamData.driver,
         "nameOftheOtherPerson": this.state.driverName
      }
   ...
   GrabTheRider = () => {
    return (
        <Box bg={'#fff'} h={230} w={'100%'} px={8} zIndex={100}
            position={'absolute'} bottom={0} borderColor={'#fff'} shadow={2}
            borderWidth={1} borderTopRightRadius={30} borderTopLeftRadius={30}>
            <Box w={'50px'} h={1} bg={'#dddddd'} alignSelf={'center'} my={2}></Box>
            <Box flexDir={'row'} alignItems={'center'} my={2} w={'100%'} >
                <Box borderWidth={2} borderColor={'#07d29a'} borderRadius={30} p={'2px'}
                    w={'40px'} h={'40px'}
                >
                   { this.state.driverPic != null? (
            <Avatar
            size="sm"
              source={{  uri:  this.state.driverPic}}
              activeOpacity={0.7}
            />
          ) :  (
            ...
            resizeMode={'cover'}
              ...
              />
                ...
                <Box w={'85%'} flexDir={'row'} alignItems={'center'} justifyContent={'space-between'}>
                    <Box ml={3} >
                        <Text
...
/>):null}
            ...
            <Box flexDir={'row'} alignItems={'center'} justifyContent={'space-between'}
                ...
                <Box flexDir={'row'} alignItems={'center'}>
                    <Pressable
                        w={'40px'} h={'40px'} borderWidth={1} borderColor={'#fff'} shadow={5} bg={'#f7f7f7'}
                        borderRadius={30} alignItems={'center'} justifyContent={'center'}
                     onPress={() => this.chat()} mr={4}
                    >
                        <Image
                            source={require('../../assets/images/ic_message_24px.png')}
                            alt={'_icon1'} resizeMode={'contain'}
                        />
                    </Pressable>
                    <Text
            ...
            {/* <Button
               ...
               // onPress={() => nav.navigate('MiddleOfWork')}
    ...
    return (
      ...
      <View style={styles.mainContainer}>
        ...
        <Header 
           ...
           leftComponent={{icon:'ios-arrow-back', type:'ionicon', color: colors.BLACK, size: 30, component: TouchableWithoutFeedback,onPress: ()=>{this.goBack()} }}
               ...
               {this.state.bookingStatus == "NEW"?null:
                <View style={styles.cabDetails}>
                    ...
                    <Text style={styles.billAmount}>
                      {Currency}{' '}
                      { this.state.estimate
                        ? parseFloat(this.state.estimate).toFixed(2)
                        : 0}
                    </Text>
                  </View>
                    ...
                    <View style={styles.cabNumber}>
                       {/*    <Text style={styles.cabNumberText}>{this.state.estimate?this.state.estimate:null}</Text>*/}
                    </View>
...
const styles = StyleSheet.create({
    ...
    topContainer: { flex: 1.5, flexDirection: 'row',borderTopWidth:0, alignItems: 'center', backgroundColor: '#EEE', paddingEnd: 20 },
    topLeftContainer: { 
        flex: 1.5, 
        alignItems:'center' 
    },
    ...
    otpContainer: {flex:0.8, backgroundColor:colors.GREEN2, width: width, flexDirection: 'row', justifyContent:'space-between' },
